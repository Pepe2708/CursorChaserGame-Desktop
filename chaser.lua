...
function Chaser:new(image, x, y, speed, rotationSpeed, jumpingSpeed)
	Chaser.super.new(self, image, x, y, speed, rotationSpeed)
...
function Chaser:update(dt)
	...
	if not self.jumping then self:calculateAngleToPlayer(love.mouse.getPosition())
...
function Chaser:moveAutomatically(dt)
	...
	if self.movingClockwise then self:rotate(-self.rotationSpeed, dt)
	elseif not self.movingClockwise then self:rotate(self.rotationSpeed, dt)
	end
